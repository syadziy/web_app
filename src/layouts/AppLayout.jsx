...
const navigation = [
  ...
  { to: '/tenants', labelKey: 'tenants', icon: 'domain', permissions: [PERMISSIONS.TENANT_VIEW] },
  ...
  { to: '/gateway-logs', labelKey: 'gatewayLogs', icon: 'receipt_long', permissions: [PERMISSIONS.AUDIT_READ] },
]
export default function AppLayout() {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand__mark">C</span><div><strong>Control Room</strong><small>Service operations</small></div></div><nav aria-label={t('mainNavigation')}>{visibleNavigation.map(({ to, labelKey, icon }) => <NavLink key={to} to={to} end={to === '/'}><MaterialIcon name={icon} />{t(labelKey)}</NavLink>)}</nav><div className="sidebar__foot"><span className="system-pulse" /> API Gateway<p>localhost:9001</p></div></aside><div className="workspace"><header className="topbar"><div><span className="breadcrumb">Control Room / {currentLabel}</span><h1>{currentLabel}</h1></div><div className="user-menu"><ThemeSelector /><LanguageSelector />{can(PERMISSIONS.ALERT_READ_NOTIFICATIONS) && <RealtimeNotifications />}<span>{session?.username || session?.roles?.[0] || 'Operator'}</span><button onClick={logout}>{t('logout')}</button></div></header><main><Outlet /></main></div></div>
}
