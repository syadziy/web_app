...
export function NotificationPanel({ notifications, unread, connectionState, open, onToggle, onClear }) {
  return <div className="realtime-notifications">
    <button className="notification-trigger" onClick={onToggle} aria-label="Notifikasi realtime" aria-expanded={open}>
      <span aria-hidden="true">♢</span>
      {unread > 0 && <strong>{unread > 99 ? '99+' : unread}</strong>}
    </button>
    {open && <section className="notification-panel" aria-label="Daftar notifikasi realtime">
      ...
      <div className="notification-list">
        {!notifications.length && <p>Belum ada alert realtime.</p>}
