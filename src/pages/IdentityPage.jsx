...
function MultiSelect({ label, options, selected, onChange, emptyText, selectText }) {
...
export default function IdentityPage() {
  ...
  return <div className="page-stack">
    ...
    <section className="page-heading"><div><p className="eyebrow">{t('userManagement')}</p><h2>{t('identityAccess')}</h2><p>{t('identityIntro')}</p></div>{canCreate && <Button onClick={openCreate}>+ {t('addItem', { item: current.item })}</Button>}</section>
    <div className="stat-strip"><div><span>Tenant</span><strong>{session.tenantId?.slice(0, 8)}…</strong></div><div><span>{t('users')}</span><strong>{users.data.length}</strong></div><div><span>{t('roles')}</span><strong>{roles.data.length}</strong></div><div><span>{t('permissions')}</span><strong>{permissions.data.length}</strong></div></div>
    ...
    <Modal title={current.title} open={modal} onClose={() => setModal(false)}><form className="form-grid" onSubmit={submit}>
      ...
      {tab === 'users' && <MultiSelect label={t('roles')} selectText={t('selectLabel', { label: t('roles').toLowerCase() })} options={roles.data.map((role) => ({ value: role.roleId, label: role.name, description: role.description || t(role.systemRole ? 'systemRole' : 'customRole') }))} selected={form.roleIds || []} onChange={(roleIds) => setForm({ ...form, roleIds })} emptyText={t('noRoles')} />}
      {tab === 'roles' && <MultiSelect label={t('permissions')} selectText={t('selectLabel', { label: t('permissions').toLowerCase() })} options={permissions.data.map((permission) => ({ value: permission.authority, label: permission.authority, description: permission.description }))} selected={form.permissions || []} onChange={(selectedPermissions) => setForm({ ...form, permissions: selectedPermissions })} emptyText={t('noPermissions')} />}
