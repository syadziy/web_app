...
function SchedulerModal({ kind, form, setForm, saving, onClose, onSubmit, t }) {
  return <Modal open={Boolean(kind)} title={t(kind === 'task' ? 'createHttpTask' : 'createSchedule')} onClose={onClose}>
...
export default function SchedulerPage() {
  ...
  const submit = async (event) => { event.preventDefault(); setSaving(true); try { const payload = { ...form }; if (kind === 'task') { payload.headers = {}; if (!payload.requestBody) delete payload.requestBody; const result = await schedulerApi.createTask(payload); setCreatedTasks((items) => [result, ...items]); tasks.reload() } else { if (payload.targetType === 'TASK') delete payload.groupId; else delete payload.taskId; await schedulerApi.createSchedule(payload); schedules.reload() } notice.success(t('itemCreated', { item: kind === 'task' ? t('tasks') : t('schedule') })); setKind(null); history.reload() } catch (error) { notice.fail(error.message) } finally { setSaving(false) } }
  ...
  return <div className="page-stack">
    ...
    <section className="page-heading"><div><p className="eyebrow">{t('automationEngine')}</p><h2>{t('scheduler')}</h2><p>{t('schedulerIntro')}</p></div>{canManage && <div className="button-row"><Button variant="secondary" onClick={() => open('schedule')}>+ {t('schedule')}</Button><Button onClick={() => open('task')}>+ Task</Button></div>}</section>
    ...
    {canManage && <SchedulerModal kind={kind} form={form} setForm={setForm} saving={saving} onClose={() => setKind(null)} onSubmit={submit} t={t} />}
  </div>
}
