...
export default function AlertsPage() {
    ...
    const recipients = useRemoteList((signal) => alertApi.recipients({ limit: 100, offset: 0 }, signal))
    const [historyResult, setHistoryResult] = useState('')
    const deliveryHistory = useRemoteList((signal) => alertApi.deliveryHistory({ ...(historyResult ? { result: historyResult } : {}), limit: 100, offset: 0 }, signal), historyResult)
    ...
    const recipientColumns = [
        ...
        { key: 'enabled', label: 'Status', render: (row) => <button className={`toggle ${row.enabled ? 'toggle--active' : ''}`} role="switch" aria-checked={row.enabled} onClick={() => toggleRecipient(row)}><i />{row.enabled ? 'Active' : 'Inactive'}</button> },
    ...
    return <div className="page-stack">
        ...
        </div> : tab === 'recipients' ? <Panel title="Recipient configuration" eyebrow="DATABASE DIRECTORY" actions={<Button onClick={() => openRecipient()}>+ Add recipient</Button>}><div className="recipient-summary"><div><strong>{recipients.data.length}</strong><span>Total recipients</span></div><div><strong>{activeRecipients.length}</strong><span>Active delivery</span></div><div><strong>{recipients.data.filter((item) => item.type === 'CC').length}</strong><span>CC recipients</span></div><div><strong>{recipients.data.filter((item) => item.type === 'BCC').length}</strong><span>BCC recipients</span></div></div><Status loading={recipients.loading} error={recipients.error} empty={!recipients.data.length} onRetry={recipients.reload} />{!recipients.loading && !recipients.error && recipients.data.length > 0 && <DataTable rows={recipients.data} columns={recipientColumns} rowKey="id" />}</Panel> : <Panel title="Email delivery history" eyebrow="DELIVERY ATTEMPTS" actions={<Button variant="ghost" onClick={deliveryHistory.reload}>Refresh</Button>}><div className="filters"><Field as="select" label="Result" name="result" options={[{ value: '', label: 'Semua' }, { value: 'SUCCESS', label: 'Success' }, { value: 'FAILED', label: 'Failed' }]} value={historyResult} onChange={(event) => setHistoryResult(event.target.value)} /></div><Status loading={deliveryHistory.loading} error={deliveryHistory.error} empty={!deliveryHistory.data.length} onRetry={deliveryHistory.reload} />{!deliveryHistory.loading && !deliveryHistory.error && deliveryHistory.data.length > 0 && <DataTable rows={deliveryHistory.data} columns={historyColumns} rowKey="id" />}</Panel>}
        <Modal title={editingId ? 'Edit recipient' : 'Add recipient'} open={recipientModal} onClose={() => setRecipientModal(false)}><form className="form-grid" onSubmit={saveRecipient}><Field label="Source system" name="sourceSystem" required value={recipientForm.sourceSystem} onChange={(e) => setRecipientForm({ ...recipientForm, sourceSystem: e.target.value })} hint="Gunakan * untuk konfigurasi global" placeholder="*" /><Field label="Display name" name="displayName" value={recipientForm.displayName} onChange={(e) => setRecipientForm({ ...recipientForm, displayName: e.target.value })} placeholder="Operations team" /><Field label="Email address" name="email" type="email" required value={recipientForm.email} onChange={(e) => setRecipientForm({ ...recipientForm, email: e.target.value })} placeholder="ops@example.com" /><Field as="select" label="Recipient type" name="type" options={[{ value: 'TO', label: 'TO — Primary recipient' }, { value: 'CC', label: 'CC — Carbon copy' }, { value: 'BCC', label: 'BCC — Blind carbon copy' }]} value={recipientForm.type} onChange={(e) => setRecipientForm({ ...recipientForm, type: e.target.value })} /><div className="form-actions"><Button type="button" variant="ghost" onClick={() => setRecipientModal(false)}>Batal</Button><Button disabled={saving}>{saving ? 'Menyimpan…' : 'Simpan recipient'}</Button></div></form></Modal>
    </div>
}
